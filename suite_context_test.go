...
func (tc *godogFeaturesScenario) iRunFeatureSuiteWithTagsAndFormatter(filter string, fmtFunc FormatterFunc) error {
	...
	traceStorage := false
	if traceStorage {
		tc.testedSuite.storage = storage.NewStorageWithTrace()
	} else {
...
func (tc *godogFeaturesScenario) followingStepsShouldHave(status string, steps *DocString) error {
	...
	err := testutils.VDiffLists(expected, actual)
	return err
}

func sorted(actual []string) {
...
func (tc *godogFeaturesScenario) theRenderJSONWillBe(docstring *DocString) error {
	...
	if err != nil {
		testutils.VDiffString(expectedString, actualString)
	}
	return err
}
...
func TestTestSuite_Run(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			expectedLines := trimAllLines(tc.log)
			actualLines := trimAllLines(log)
			assert.Equal(t, expectedLines, actualLines, log)
		})
	}
}
