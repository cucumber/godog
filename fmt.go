...
func NewBaseFmt(suite string, out io.Writer, snippetFunc string) *BaseFmt {
	return internal_fmt.NewBase(suite, out, snippetFunc)
}

// NewProgressFmt creates a new progress formatter.
func NewProgressFmt(suite string, out io.Writer, snippetFunc string) *ProgressFmt {
	return internal_fmt.NewProgress(suite, out, snippetFunc)
}

// NewPrettyFmt creates a new pretty formatter.
func NewPrettyFmt(suite string, out io.Writer, snippetFunc string) *PrettyFmt {
	return &PrettyFmt{Base: NewBaseFmt(suite, out, snippetFunc)}
}

// NewEventsFmt creates a new event streaming formatter.
func NewEventsFmt(suite string, out io.Writer, snippetFunc string) *EventsFmt {
	return &EventsFmt{Base: NewBaseFmt(suite, out, snippetFunc)}
}

// NewCukeFmt creates a new Cucumber JSON formatter.
func NewCukeFmt(suite string, out io.Writer, snippetFunc string) *CukeFmt {
	return &CukeFmt{Base: NewBaseFmt(suite, out, snippetFunc)}
}

// NewJUnitFmt creates a new JUnit formatter.
func NewJUnitFmt(suite string, out io.Writer, snippetFunc string) *JUnitFmt {
	return &JUnitFmt{Base: NewBaseFmt(suite, out, snippetFunc)}
}
