...
func TestScenarioContext_Step(t *testing.T) {
	...
	for _, c := range []tc{
		...
		{n: "ScenarioContext should panic if step handler is not a function",
			p: `expected handler for ".*" to be func, but got: int`,
			f: func() { ctx.Step(".*", 124) }},
		{n: "ScenarioContext should panic if step handler has more than 2 return values",
			p: `expected handler for ".*" to return either zero, one or two values, but it has: 3`,
			f: func() { ctx.Step(".*", nokLimitCase3) }},
		{n: "ScenarioContext should panic if step handler has more than 2 return values (5)",
			p: `expected handler for ".*" to return either zero, one or two values, but it has: 5`,
		...
		{n: "ScenarioContext should panic if step return type is []string",
			p: `expected handler for ".*" to return one of error or context.Context or godog.Steps or (context.Context, error), but got: []string`,
		...
		{n: "ScenarioContext should panic if step handler return type is not an error or string slice or void (interface)",
			p: `expected handler for ".*" to return one of error or context.Context or godog.Steps or (context.Context, error), but got: interface {}`,
		...
		{n: "ScenarioContext should panic if step handler return type is not an error or string slice or void (slice)",
			p: `expected handler for ".*" to return one of error or context.Context or godog.Steps or (context.Context, error), but got: []int`,
		...
		{n: "ScenarioContext should panic if step handler return type is not an error or string slice or void (other)",
			p: `expected handler for ".*" to return one of error or context.Context or godog.Steps or (context.Context, error), but got: chan int`,
